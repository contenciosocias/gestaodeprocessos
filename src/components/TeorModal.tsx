...
import { formatDateBR, limparTeor } from '../lib/format'

export function TeorModal({ intimacao, onClose }: { intimacao: IntimacaoComProcesso | null; onClose: () => void }) {
  return (
    <Modal open={Boolean(intimacao)} onClose={onClose} title="Inteiro teor">
      {intimacao && (
        <div className="space-y-4">
          <dl className="grid grid-cols-1 gap-x-6 gap-y-2 text-sm sm:grid-cols-2">
            <Campo rotulo="Processo" valor={intimacao.numero_processo || intimacao.processo?.numero_cnj} />
            <Campo
              rotulo="Tribunal / Órgão"
              valor={[intimacao.sigla_tribunal, intimacao.nome_orgao].filter(Boolean).join(' · ')}
            />
          ...
          <div>
            ...
            <div className="max-h-[55vh] overflow-y-auto whitespace-pre-wrap rounded-lg border border-cias-borda bg-cias-superficie p-4 text-sm leading-relaxed text-cias-texto">
              {limparTeor(intimacao.teor) || 'Sem teor disponível.'}
            </div>
          </div>
