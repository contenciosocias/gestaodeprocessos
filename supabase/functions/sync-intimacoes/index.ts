...
// ----------------------------------------------------------------------------
// Puxa as intimações do DJEn (Comunica PJe) PARA CADA processo cadastrado e
// mantém só as dirigidas às OABs monitoradas. Faz upsert ON CONFLICT DO NOTHING
// (o conteúdo do DJEn é imutável para nós; assim status/prazo_fatal/observacao,
// editados por nós, nunca são sobrescritos).
//
// Contrato confirmado AO VIVO contra a produção:
...
//             (numero_oab vem como STRING e SEM zeros à esquerda; pode ser '')
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (oabSet.size === 0) {
      ...
      return jsonResponse({ ok: true, aviso: 'Nenhuma OAB monitorada cadastrada.', processos_consultados: 0, intimacoes_novas: 0 })
    }
...
async function touchSyncState(supabase: ReturnType<typeof createClient>): Promise<void> {
